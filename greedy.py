...
class BirdSortGreedy:
    ...
    def save_result(self):
        ...
        result_data = f"""Algorithm: Greedy
Difficulty: {self.difficulty}             
Colors: {num_colors}
Branches: {num_branches}
Time Taken: {self.elapsed_time:.6f}s
States Explored: {self.states_explored}
Avg Branching Factor (~ moves/step)= {self.avg_branching_factor:.2f}
Solution Steps:
""" + "\n".join(f"{i+1}. {step}" for i, step in enumerate(self.solution))
        
        filename = f"game_states/results/greedy/GREEDY_difficulty={self.difficulty}_exectime={self.elapsed_time:.5f}.txt"
