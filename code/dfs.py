...
class BirdSortDFS:
    ...
    def create_controls(self):
        ...
        self.stats_label = tk.Label(self.root, text="Empty Branches = 0, States Explored = 0, Max Depth = 0", font=("Arial", 11))
...
""" + "\n".join(f"{i+1}. {step}" for i, step in enumerate(self.solution))
        ...
        filename = f"../results/dfs/DFS_difficulty={self.difficulty}_exectime={self.elapsed_time:.5f}.txt"
    ...
    def solve(self):
        print("Starting DFS...")
        stack = [(copy.deepcopy(self.branches), [], 0)]
        visited = set()
